...
def upload_new_rows_to_sheet(csv_path, config_path="config.yml"):
...
def spending_report(csv_path, config_path="config.yml"):
    ...
    # ✅ Get or create the worksheet
    report_tab_name = config["google"]["worksheet_name"] + "-report"
    ...
    df = pd.read_csv("final_output/all_cards.csv")
    ...
    df.sort_values(by='Amount', ascending=False)
      .head(5)
      .drop(columns=['Card', 'Card Owner', 'Source File'])
    )
    ...
    category_data.append(['Spending by Category'])
    category_data.append(['Category', 'Amount'])
    category_data += spending_by_category.values.tolist()
    category_data.append([])
    
    report_data.append(['Total Spent:', total_spent])
    report_data.append([])
    
    top_5_data.append(['5 Largest Purchases'])
    top_5_data.append(largest_purchases.columns.tolist())
    top_5_data += largest_purchases.values.tolist()
    print (largest_purchases.values)
    ...
    report_ws = get_or_replace_worksheet(spreadsheet, title=report_tab.title, rows=100, cols=10)

    
    # --- Upload the report
    report_ws.update('B1', report_data)
    report_ws.update('E1', category_data)
    report_ws.update('A20', top_5_data)


    print("🧬 Applying Formatting")
    header = cellFormat(
    # backgroundColor=color(1, 0.9, 0.9),
    textFormat=textFormat(bold=True, fontSize=18, foregroundColor=color(1, 0, 1)),
    ...
    format_cell_range(report_ws, 'A:F', centered)
    
    format_cell_range(report_ws, 'F3:F', currency_format)
    format_cell_range(report_ws, 'C22:C26', currency_format)
    format_cell_range(report_ws, 'C1', currency_format)

    format_cell_range(report_ws, 'B1', header)
    format_cell_range(report_ws, 'E1', header)
    format_cell_range(report_ws, 'A20', header)

    # worksheet.update_tab_color(color(1, 0.9, 0.9))

    set_column_widths(report_ws, [ ('A', 75), ('B', 300), ('C', 90), ('D', 150), ('E', 150), ('F', 135), ('G', 150), ('H:', 50) ])
